...
test("a document recorded but not yet batched is not 'never seen'", async () => {
   ...
   * which is an hour by default. Through all of it the gateway answered 404
   * and "this gateway has no record of that document" — the same answer a
   * document nobody has ever heard of gets — while the signed receipt for it
   * sat in this gateway's own store.
   *
   * It is the one output this endpoint's whole design says must never be
   * produced carelessly: a verifier who acts on "no record" does something
   * irreversible, and here they would be acting on a document accepted
   * minutes earlier. The endpoint is careful about "could not check" for
   * exactly that reason; it was not careful about this.
   */
  ...
  try {
    ...
    // The receipt is the whole point: it exists, so it is served.
    ...
    // And `batch` being null is what tells this apart from a batched
    // document that is merely not anchored yet.
...
test("the same is true with chain access, where the contract has not heard of it either", async () => {
...
test("a batched document is not-anchored, where an unbatched one is receipted", async () => {
  // The two share a status, so the wording has to carry the difference: one
  // has an inclusion proof to check and the other has nothing yet.
  const gw = await startGateway({ reader: stubChain() });
  try {
    const { document } = await recorded(gw); // built this time
    const body = await (await verify(gw, document.fileHash)).json();

    assert.equal(body.gatewayClaim.status, "not-anchored", "the next state along, not the same one");
    assert.ok(body.batch, "a built batch is served with its proof");

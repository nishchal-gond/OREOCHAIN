...
test("no server module writes a refusal code by hand", () => {
   ...
   * other than the one that defines them, where nothing compared them to
   * anything — so a rename would have moved the export and the client together
   * and left the quota refusals behind, sending a code the client has never
   * heard of, from the two routes most likely to refuse.
   *
   * It could not import them: the gateway imports quota, so quota importing the
   * gateway back is a cycle. Hence server/refusals.mjs, which every module can
   * import, and this, which notices the next one.
   */
  ...
  assert.deepEqual(
    offenders,
    [],
    "these write a refusal code as a literal instead of importing REFUSAL_CODES"
  );
});

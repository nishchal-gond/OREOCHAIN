 ...
 *
 * It lives in its own module so every server module can import it without
 * importing the gateway. server/quota.mjs is why: the gateway imports quota,
 * so quota cannot import the gateway back without a cycle, and it was writing
 * two of these seven as bare strings instead — in a module other than the one
 * that defines them, where nothing compared them to anything. A rename would
 * have moved the export and the browser client together and left the quota
 * refusals behind, sending a code the client has never heard of, from the two
 * routes most likely to refuse.
 *
 * server/gateway.mjs re-exports it under the same name, because that export is
 * what the browser client's cross-check imports.
 *
 ...
 * asserts the two agree, and that no server module writes one by hand.
 */
